        ...
        {GameList.map(
          (item, index) =>
            index < 4 && (
              <div
                key={item.id}
                className="bg-[#222557] rounded-lg group hover:scale-110 transition-all duration-300 ease-in-out"
              >
                <img
                  src={item.background_image}
                  className="h-[270px] rounded-lg object-cover"
                />
                <h2 className="dark:text-white text-[20px] p-2 font-bold text-white">
                  {item.name}
                </h2>
              </div>
            )
